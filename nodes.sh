...
#
# Env vars consumed (all optional; sensible defaults below):
#   RC_SSH_HOST          [user@]host       — the deploy target
#   RC_SSH_PORT          port              — non-22 SSH port (e.g. 2222 for
#                                            the local test container)
#   SSH_KEY              path              — EC2 key pair private key
#   RC_SSH_EXTRA_OPTS    "-o foo=bar ..."  — passed through to ssh/scp
#                                            (e.g. UserKnownHostsFile=/dev/null
#                                            for local tests)

RC_SSH_HOST="${RC_SSH_HOST:-rc@ec2-CHANGEME.compute-1.amazonaws.com}"
