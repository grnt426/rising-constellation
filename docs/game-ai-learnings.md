...
| 07-21 | Champion FRONTIER flat 3 weeks (p90 fitness ~400, cp75 sys=3) while median doubled; 6-sys empires win 76% vs 2-sys 56% — champions at a local max | Root cause: colony fitness `40·ln(50n)` saturates (+34 for 4th–7th vs 300 win bonus), no gradient to expand. Consolidated round-4c (hard-code quality_siting incl. frontier-only win; kill expansion_ideo_share); report now frontier-first | quality_siting frontier fit 446 vs 386, sys 2→3 (undersold by the mean); expansion_ideo_share reversed at 24h (zero-col 50% vs 30%) |
| 07-21 | VP-centric fitness rewards hollow timeout wins over strong losses; no empire-quality gradient (user redesign) | `Headless.Fitness`: dense empire-first score — golden-line empire (400) + LINEAR colonies (250) + diminishing 6-mechanic breadth (200) + VP/win lowest (100) − hoard penalty (150) − hollow-timeout penalty; ~1000 = golden-line generalist decisive win. `mix headless.rescore` re-rulers the archive from stored stats | ordering tests pass (hollow win < good loss < generalist win; 3→6 sys +150) |
