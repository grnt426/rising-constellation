...
ownership = %{
  systems: Map.new(own_raw["systems"], fn s -> {s["id"], %{faction: s["faction"], status: s["status"]}} end),
...
sys = fn name ->
  case Map.get(name_index, name) do
    nil ->
      IO.puts("WARN: system #{name} not found")
      nil
...
stress_records =
  [
    ...
    {"Drell", "myrmezir", 1, 4},
    {"Ixara", "myrmezir", 1, 1},
    {"Halcyon", "tetrarchy", 5, 5},
    {"Petraeus", "tetrarchy", 3, 4},
    {"Winnow", "tetrarchy", 2, 3},
    {"Sarland", "tetrarchy", 2, 2},
    {"Jassa", "tetrarchy", 1, 5},
    {"Mordune", "tetrarchy", 1, 2}
  ]
...
stress_highlights =
  ([
     ...
     hl.("Cone", :bombard, []),
     hl.("Quere", :bombard, []),
     hl.("Dumfri", :conquest, faction: "cardan"),
     hl.("Rimsby", :conquest, faction: "myrmezir")
   ] ++
     Enum.map(
       [
         {"Amorin", 3},
  ...
  Cards.digest(%{
    ...
    vp: %{
      ...
      rows: [
        ...
        %{
          faction: "synelle",
          vp: by_key["synelle"]["victory_points"],
          gained: [],
          lost: [by_key["synelle"]["victory_points"] + 1]
        }
      ]
    },
    totals: [
...
# --- E. /player profile card ------------------------------------------
# Needs no instance fixture — profile flavor + stats only. The avatar
# read degrades to the initial-letter placeholder if priv/data/avatars
# is missing.

avatar =
  case File.read(Path.join([to_string(:code.priv_dir(:rc)), "data/avatars", "avatarF_003.jpg"])) do
    {:ok, jpeg} -> jpeg
    _ -> nil
  end
...
render.(
  ...
  Cards.player_profile(%{
    name: "Alrua",
    full_name: "Grand Archon Alrua of the Violet Reach",
    description: "The stars remember what the senate forgets.",
    avatar_jpeg: avatar,
    favorite_faction: "cardan",
    favorite_icon: "ship/frigate_1",
    stats: %{
      legacy: %{wins: 2, participations: 5},
      daily: %{gold: 3, silver: 7, bronze: 4, completed: 38, played: 51},
      factions: %{"cardan" => 9, "tetrarchy" => 3, "ark" => 1}
    }
  })
)
